...
//		for (ListIterator<SimulationEventListener> itr = simulationEventListenerList.listIterator(); itr.hasNext(); )

		{
		
			
			// surround existing code with exception handling code to find error 
//			System.out.println("before simulationEventOccurred invocation in AntGUI, size = " 
//					+ simulationEventListenerList.size() + " simEvent = " + eventType);
			
			try {

			// System.out.println((SimulationEventListener)itr);
			((SimulationEventListener)itr.next()).simulationEventOccurred(simEvent);
//				itr.next().simulationEventOccurred(simEvent);
